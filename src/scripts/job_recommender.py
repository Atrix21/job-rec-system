...
# Add a very generic job posting
kg.add_job(
    "job_generic",
    "Generalist",
...
# Add skill similarity edges
kg.add_skill_similarity_edges()
# Add job-to-job similarity edges
kg.add_job_to_job_similarity_edges(threshold=0.7)

# Add resumes (including Zara)
resumes = [
    {
        'resume_id': 'resume_1',
        'name': 'Alice',
...
# Upsert all jobs into Qdrant using integer IDs
...
# Upsert all resumes into Qdrant (new collection 'resumes') using integer IDs
...
print("\n--- Sample Job Postings ---")
for i in range(3):
    job = kg.jobs[f"job_{i+1}"]
    print(f"Job ID: job_{i+1}")
    print(f"Title: {job['title']}")
    print(f"Description: {job['description']}")
    print(f"Required Skills: {job['required_skills']}")
    print(f"Optional Skills: {job['optional_skills']}")
    print("-" * 60)
# Print the generic job
job = kg.jobs["job_generic"]
print(f"Job ID: job_generic")
print(f"Title: {job['title']}")
print(f"Description: {job['description']}")
print(f"Required Skills: {job['required_skills']}")
print(f"Optional Skills: {job['optional_skills']}")
...
# --- Print synthetic resumes ---
print("\n--- Synthetic Resumes ---")
for r in resumes:
    print(f"Resume ID: {r['resume_id']}")
    print(f"Name: {r['name']}")
    print(f"Summary: {r['summary']}")
    print(f"Priority Skills: {r['priority_skills']}")
    print(f"Other Skills: {r['other_skills']}")
    print("-" * 60)

# --- Print embedding example ---
print("\n--- Embedding Example ---")
print("Job Embedding (first 10):", kg.job_embeddings['job_1'][:10])
print("Resume Embedding (first 10):", kg.resume_embeddings['resume_1'][:10])

# --- Recommend jobs for each resume ---
SOFT_SKILLS = {
...
for r in resumes[:5]:
    print(f"\n=== Recommendations for {r['name']} ({r['resume_id']}) ===")
    # KG Overlap (weighted, at least one hard skill match)
    print("-- KG Overlap (weighted, hard skill required) --")
    resume_skills = set(r['priority_skills'] + r['other_skills'])
    job_scores_overlap = {}
    for job_id, job in kg.jobs.items():
        score = 0.0
            ...
            if skill in resume_skills:
                ...
                for sim in kg.skill_synonyms.get(skill, []):
                    if sim in resume_skills:
                        ...
                        if not is_soft_skill(skill):
                            hard_skill_matched = True
        total = sum(1.0 if not is_soft_skill(s) else 0.5 for s in job['required_skills']) + \
                sum(0.5 if not is_soft_skill(s) else 0.25 for s in job['optional_skills'])
        if total > 0 and hard_skill_matched:
            job_scores_overlap[job_id] = score / total
    ranked_overlap = sorted(job_scores_overlap.items(), key=lambda x: x[1], reverse=True)[:5]
    for jid, score in ranked_overlap:
        print(f"{kg.jobs[jid]['title']} (score: {score:.3f})")
    # PageRank
    print("-- PageRank --")
    seeds = {n: 1.0 if n in resume_skills else 0.0 for n in kg.graph.nodes}
    pr = nx.pagerank(kg.graph, personalization=seeds, alpha=0.85, max_iter=100)
    job_scores_pr = {jid: pr[jid] for jid in kg.jobs if jid in pr}
    ranked_pr = sorted(job_scores_pr.items(), key=lambda x: x[1], reverse=True)[:5]
    for jid, score in ranked_pr:
        print(f"{kg.jobs[jid]['title']} (score: {score:.6f})")
    # Qdrant vector search (job embedding, search resumes)
    print("-- Qdrant Vector Search --")
    job_emb = kg.job_embeddings[job_id]
    qdrant_results = qdrant.search_resume(job_emb, top_k=5)
    for res in qdrant_results:
        resume_id = int_to_resume_id[int(res['resume_id'])]
        candidate = next(resume for resume in resumes if resume['resume_id'] == resume_id)
        print(f"{candidate['name']} (score: {res['score']:.6f})")
    ...
    # Get overlap and PageRank scores (already computed above)
    alpha, beta = 0.4, 0.4
    hybrid_scores = {}
    for job_id in kg.jobs:
        overlap = job_scores_overlap.get(job_id, 0.0)
        pr_score = job_scores_pr.get(job_id, 0.0)
        qdrant_score = qdrant_score_map.get(job_id, 0.0)
        # Only consider jobs with at least one hard skill match (as in overlap)
        if job_id in job_scores_overlap:
            hybrid_scores[job_id] = alpha * overlap + beta * pr_score + (1 - alpha - beta) * qdrant_score
    ranked_hybrid = sorted(hybrid_scores.items(), key=lambda x: x[1], reverse=True)[:5]
    for jid, score in ranked_hybrid:
        print(f"{kg.jobs[jid]['title']} (score: {score:.3f})\n  Desc: {kg.jobs[jid]['description'][:120]}...") 